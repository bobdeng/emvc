...
public class BytesUtils {
	...
	throws IOException {
		...
		short sLen = (strMsg == null) ? 0 : (short) strMsg.getBytes("utf-8").length;
		writeShort(stream, sLen);
		if (sLen > 0)
	...
	public static void main(String[] args) throws IOException {
		...
		ByteArrayInputStream tempi=new ByteArrayInputStream(temp.toByteArray());
		System.out.println(readLong(tempi, 8));
	}
	public static byte[] readByteArray(InputStream input)
	throws IOException
	{
		int len=BytesUtils.ReadInt(input,2);
		return readByteArray(input,len);
	}
